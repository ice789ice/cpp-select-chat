...
## 面试讲解版本（简要话术）
- **问题背景**：实现一个可同时处理多客户端聊天的 TCP 服务端。
- **方案选择**：使用 `select()` 做 I/O 多路复用，单线程事件驱动，避免多线程开销。
- **关键细节**：处理 `EINTR`、`EPIPE`，避免 `FD_SETSIZE` 越界；用 `select` 超时实现空闲连接清理。
- **权衡与扩展**：`select` 存在 O(n) 扫描与 fd 上限，后续可升级为 `poll/epoll` 并拆分连接管理模块。
